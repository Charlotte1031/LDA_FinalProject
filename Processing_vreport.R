...
FilteredData <- MergedData %>%
  filter(!is.na(AL)) %>%  # Remove rows where SER is NA
...
ggplot(SampledData, aes(x = Visit, y = AL, group = PtID, color = as.factor(PtID))) +
  geom_line(aes(linetype = LineType), size = 0.5, alpha = 0.7) +  # Line for each patient with line type by group
  geom_point(size = 2, shape = 1) +                                        # Add points at each SER measurement
  labs(title = "SER Progression Over Time by Treatment Group",
       x = "Visit",
...
MeanProfileData <- MergedData %>%
  filter(Visit != "Run-in FU Randomization") %>%
  group_by(Visit, TrtGroup) %>%
  summarize(Mean_SER = mean(SER, na.rm = TRUE),Mean_AL = mean(AL, na.rm = TRUE), .groups = 'drop')

# Combined plot with individual trajectories and mean profiles
# The one used for report
ggplot() +
  # Individual trajectories
  geom_line(data = SampledData, 
            aes(x = Visit, y = AL, group = PtID, color = as.factor(PtID), 
                linetype = TrtGroup),
            size = 0.5, alpha = 0.7) +
  geom_point(data = SampledData,
             aes(x = Visit, y = AL, color = as.factor(PtID)),
             size = 2, shape = 1) +
  # Mean profiles
  geom_line(data = MeanProfileData,
            aes(x = Visit, y = Mean_AL, group = TrtGroup, linetype = TrtGroup),
            size = 1.2) +
  geom_point(data = MeanProfileData,
             aes(x = Visit, y = Mean_AL, color = TrtGroup),
             size = 3, shape = 21, fill = "white") +
  # Labels and theme
  labs(title = "Axial Length Progression Over Time by Treatment Group",
       x = "Visit",
       y = "Axial Length",
       color = "Patient ID",
       linetype = "Treatment Group") +
...
data <- MergedData[,c("PtID","Visit","SER","AL","TrtGroup","MotherMyop","FatherMyop","Sex","Race","EyeColor","AgeAsofEnrollDt")]
...
data <- data %>% filter(Visit != "Run-in FU Randomization") 
# random sample 5 data rows
set.seed(215) # Setting seed for reproducibility
